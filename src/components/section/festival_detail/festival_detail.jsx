...
const FestivalDetail = ({ festivals, festivalRepository, authService }) => {
  ...
  useEffect(() => {
    festivals
      .contentInfo(festivalInfo.contentid)
      .then((data) => setDetails((prev) => (prev = data)));
    festivals
      .contentImg(festivalInfo.contentid)
      .then((data) => setImg((prev) => (prev = data)));
    festivals
      .contentIntro(festivalInfo.contentid)
      .then((data) => setIntros((prev) => (prev = data)));
    ...
    <>
      ...
      <section className={styles.section}>
        ...
        <div className={styles.title}>
          ...
          today < festivalInfo.eventenddate ? (
          ...
          <span className={styles.eventDate}>
            {dateChange(festivalInfo.eventstartdate)} ~{" "}
            {dateChange(festivalInfo.eventenddate)}
          </span>
        </div>
        ...
        <div className={styles.info}>
          ...
          <ul className={styles.detailInfo}>
            {more && more.length > 1
              ? more.map(
                  (more, index) =>
                    index !== 0 && (
                      <DetailInfo more={more} key={more.fldgubun} />
                    )
                )
          ...
          <div className={styles.buttonBox}>
            <button
              ...
              style={{
                display: moreBtn.current === 0 ? "inline-block" : "none",
              }}
              onClick={() => moreDetail(festivalInfo.contentid)}
            >
              내용 더보기
            ...
            <button
              className={styles.button}
              style={{
                display: moreBtn.current === 1 ? "inline-block" : "none",
              }}
              onClick={() => closeDetail()}
            >
              내용 닫기
